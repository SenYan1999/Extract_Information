...
class CRF(nn.Module):
    ...
    def __init__(self, num_tag, use_cuda=False):
        ...
        # P_j* 表示所有从tag_j出发的边
        # P_*k 表示所有到tag_k的边
        self.transitions = nn.Parameter(torch.Tensor(num_tag + 2, num_tag + 2)) # 转化为模型和训练的参数
    ...
    def viterbi(self, features):
        ...
        back_points, index_points = [self.transitions[self.start_tag, :self.start_tag]], [torch.LongTensor([-1]).cuda().expand_as(forward)]
        ...
        back_points.append(forward + self.transitions[:self.start_tag, self.end_tag])  # END_TAG
